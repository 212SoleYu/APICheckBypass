...
use std::rc::Rc;
use crate::analysis::mir_visitor::func_handler::*;
    ...
    pub fn handled_as_special_function_call(&mut self) -> bool {
        ...
        match self.callee_known_name {
            ...
            KnownNames::StdSliceSwap =>{
                let record = FuncHandler::new(
                    self.callee_def_id,
                    FuncClass::Index,
                    self.block_visitor.body_visitor.current_span,
                    self.actual_args.to_vec(),
                    KnownNames::StdSliceSwap,
                    0
                );
                self.block_visitor.body_visitor.replace_funcs.insert(record);
        ...
        match check_result {
            ...
            CheckerResult::Unsafe => {
                let error = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Provably error: index out of bound in get() call"),
                );
                error.emit();
            }
            CheckerResult::Warning => {
                let warning = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Possible error: index out of bound in get() call"),
                );
                warning.emit();
            }
        }
    ...
    fn handle_get_checked_mut(&mut self) -> bool{
        ...
        match check_result {
            ...
            CheckerResult::Unsafe => {
                let error = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Provably error: index out of bound in get_mut() call"),
                );
                error.emit();
            }
            CheckerResult::Warning => {
                let warning = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Possible error: index out of bound in get_mut() call"),
                );
                warning.emit();
            }
        }
    ...
    fn handle_split_at_checked(&mut self) -> bool{
        ...
        match check_result {
            ...
            CheckerResult::Unsafe => {
                let error = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Provably error: index out of bound in split_at_checked() call"),
                );
                error.emit();
            }
            CheckerResult::Warning => {
                let warning = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Possible error: index out of bound in split_at_checked() call"),
                );
                warning.emit();
            }
        }
    ...
    fn handle_split_at_mut_checked(&mut self) -> bool{
        ...
        match check_result {
            ...
            CheckerResult::Unsafe => {
                let error = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Provably error: index out of bound in split_at_mut_checked() call"),
                );
                error.emit();
            }
            CheckerResult::Warning => {
                let warning = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Possible error: index out of bound in split_at_mut_checked() call"),
                );
                warning.emit();
            }
        }
    ...
    fn handle_checked_add(&mut self) -> bool {
        ...
        match check_result {
            ...
            CheckerResult::Unsafe => {
                let error = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Provably error: integer overflow in checked_add()"),
                );
                error.emit();
            }
            CheckerResult::Warning => {
                let warning = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Possible error: integer overflow in checked_add()"),
                );
                warning.emit();
            }
        }
    ...
    fn handle_swap(&mut self) -> bool{
        ...
        match check_result_a {
            ...
            CheckerResult::Unsafe => {
                let error = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Provably error: first index out of bound in swap() call"),
                );
                error.emit();
            }
            CheckerResult::Warning => {
                let warning = body_visitor.context.session.dcx().struct_span_warn(
                    body_visitor.current_span,
                    format!("[MirChecker] Possible error: first index out of bound in swap() call"),
                );
                warning.emit();
            }
        }
